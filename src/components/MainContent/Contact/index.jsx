...
const contactSchema = z.object({
    ...
    phoneNumber: z.string().min(9, "Number must be 9 characters").max(11, "Number must be 9 characters").regex(/^\d+$/),
...
const Contact = () => {
    ...
    const handleInputChange = (e) => {
        ...
        let formatted = input
            .match(/.{1,3}/g) // Split into groups of 3
                        ...
                        <form onSubmit={handleSubmit(onSubmitForm)}>
                            <div className="flex space-x-2 mb-4">
                                <div className="flex-1">
                                    ...
                                    />
                                    {errors.name && (
                                        <div className="text-red-500 text-sm font-medium px-4 py-2 bg-red-50 border border-red-200 rounded-lg">{errors.name.message}</div>
                                    )}
                                </div>
                                <div className="flex-1">
                                    <input
                                    ...
                                    />
                                    {errors.email && (
                                        <div className="text-red-500 text-sm font-medium px-4 py-2 bg-red-50 border border-red-200 rounded-lg">{errors.email.message}</div>
                                    )}
                                </div>
                            </div>
                            <div className="mb-4">
                                ...
                                {errors.phoneNumber && (
                                    <div className="text-red-500 text-sm font-medium px-4 py-2 bg-red-50 border border-red-200 rounded-lg">{errors.phoneNumber.message}</div>
                                )}
                            </div>
                            <div className="mb-4">
                                <textarea
                                ...
                                ></textarea>
                                {errors.message && (
                                    <div className="text-red-500 text-sm font-medium px-4 py-2 bg-red-50 border border-red-200 rounded-lg">{errors.message.message}</div>
                                )}
                            </div>
                            <button
                                type="submit"
                                className={`w-full font-semibold text-white py-3 ${isValid ? "bg-[#ff601c]" : "bg-[#ff601caa]"
                                    } rounded-lg`}
